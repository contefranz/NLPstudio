...
create_corpus = function(df_container, form_type = c("10-K"), to_disk = FALSE, path_out = NULL) {
  ...
  for ( i_year in seq_along(df_container) ) {
    ...
    filename_df[ , checkdup := duplicated(filename_df, by = "doc_id_corpus")]
    check_not_unique_filenames = nrow(filename_df[checkdup == TRUE])
