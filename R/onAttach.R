...
.onAttach <- function(...) {
  attached <- edgartools_attach()
  message(edgartools_attach_message(attached))
}
...
edgartools_attach_message <- function(to_load) {
  if (length(to_load) == 0) {
    return(NULL)
  }
  
  header <- cli::rule(
    left = cli::style_bold("Attaching core edgartools packages"),
    right = paste0("edgartools ", package_version_h("edgartools"))
  )
